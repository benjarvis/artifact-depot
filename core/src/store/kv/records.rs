...
use super::metadata::*;
use crate::scanner::{ScanStatus, Severity, SeverityCounts};

/// Current schema version stamped on every new record.
pub const CURRENT_RECORD_VERSION: u32 = 6;
...
pub struct RepoConfig {
    ...
    #[serde(default)]
    pub deleting: bool,
    /// When true, ingested blobs are enqueued for vulnerability scanning.
    /// V5 and older records lack this field and deserialize as `false`.
    #[serde(default)]
...
impl ArtifactFormat {
    /// Inverse of [`Display`] — parse the lowercase name back to an enum
    /// value. Returns `None` for unknown strings.
    pub fn from_display(s: &str) -> Option<Self> {
        match s {
            "raw" => Some(ArtifactFormat::Raw),
            "docker" => Some(ArtifactFormat::Docker),
            "pypi" => Some(ArtifactFormat::PyPI),
            "apt" => Some(ArtifactFormat::Apt),
            "golang" => Some(ArtifactFormat::Golang),
            "helm" => Some(ArtifactFormat::Helm),
            "cargo" => Some(ArtifactFormat::Cargo),
            "yum" => Some(ArtifactFormat::Yum),
            "npm" => Some(ArtifactFormat::Npm),
            _ => None,
        }
    }
}
...
/// Result of a completed (or attempted) scan of a single blob, keyed by
/// `(scanner, blob_hash)` so results dedup across repos that share content.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScanResultRecord {
    #[serde(default)]
    pub schema_version: u32,
    /// Backend that produced this record (e.g. `"trivy"`).
    pub scanner: String,
    /// BLAKE3 hex hash of the scanned blob.
    ...
    /// BLAKE3 of the full JSON report stored as a regular blob. Empty when
    /// `status` is [`ScanStatus::Error`] and no report was produced.
    #[serde(default)]
    pub report_blob_hash: String,
    /// BLAKE3 of a generated SBOM, if the scanner produced one.
    #[serde(default)]
    pub sbom_blob_hash: Option<String>,
    pub duration_ms: u64,
    pub attempt: u32,
    /// Last error message when `status` is not `Ok`.
    #[serde(default)]
    pub last_error: Option<String>,
}

/// Provenance of an SBOM.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(tag = "source", rename_all = "lowercase")]
pub enum SbomSource {
    /// SBOM supplied by the artifact producer, via sidecar upload or an OCI
    /// referrer. Preferred over `Generated` when both exist.
    Producer {
        /// Free-form origin tag (e.g. `"oci_referrer"`, `"sidecar_upload"`,
        /// `"cosign_attestation"`).
        origin: String,
    },
    /// SBOM generated by a scanner during analysis. Stored as a byproduct of
    /// scanning so rescans don't have to regenerate it.
    Generated { scanner: String },
}

/// SBOM serialisation format.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[cfg_attr(feature = "openapi", derive(utoipa::ToSchema))]
#[serde(rename_all = "lowercase")]
pub enum SbomFormat {
    #[serde(alias = "cyclone_dx")]
    CycloneDx,
    Spdx,
}

/// SBOM metadata keyed by the target blob's BLAKE3 hash.
///
/// The SBOM bytes themselves live in the blob store (content-addressed and
/// dedup-eligible like any other blob). This record points at them.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct SbomRecord {
    #[serde(default)]
    pub schema_version: u32,
    /// BLAKE3 hex hash of the artifact this SBOM describes.
    pub target_blob_hash: String,
    /// BLAKE3 hex hash of the SBOM blob itself.
    pub sbom_blob_hash: String,
    pub source: SbomSource,
    pub format: SbomFormat,
    pub created_at: DateTime<Utc>,
}

/// OCI referrers record linking a subject manifest to an attached artifact
/// (e.g. a cosign-signed SBOM attestation).
///
/// Keyed by `(repo, subject_digest, referrer_digest)` so the referrers list
/// for any subject is a single-partition scan.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct OciReferrerRecord {
    #[serde(default)]
    pub schema_version: u32,
    /// SHA-256 digest of the subject manifest.
    pub subject_digest: String,
    /// SHA-256 digest of the referring manifest.
    pub referrer_digest: String,
    /// Artifact type declared by the referrer manifest (e.g.
    /// `application/vnd.cyclonedx+json`).
    pub artifact_type: String,
    /// Media type of the referrer manifest itself.
...
#[cfg(test)]
mod scanner_record_tests {
    use super::*;

    fn sample_scan_result() -> ScanResultRecord {
        ScanResultRecord {
            schema_version: CURRENT_RECORD_VERSION,
            scanner: "trivy".into(),
            blob_hash: "abc123".into(),
            scanner_version: "0.60.0".into(),
            vuln_db_version: "2026-04-15T00:00:00Z".into(),
            scanned_at: DateTime::<Utc>::default(),
            ...
            report_blob_hash: "deadbeef".into(),
            sbom_blob_hash: Some("cafebabe".into()),
            duration_ms: 1234,
            attempt: 1,
            last_error: None,
        }
    ...
    #[test]
    fn scan_result_record_roundtrip() {
        let rec = sample_scan_result();
        let bytes = rmp_serde::to_vec(&rec).unwrap();
        let back: ScanResultRecord = rmp_serde::from_slice(&bytes).unwrap();
        assert_eq!(back.scanner, "trivy");
        assert_eq!(back.highest_severity, Severity::Critical);
        assert_eq!(back.severity_counts.critical, 1);
    }

    #[test]
    fn sbom_record_roundtrip_producer() {
        let rec = SbomRecord {
            schema_version: CURRENT_RECORD_VERSION,
        ...
        let bytes = rmp_serde::to_vec(&rec).unwrap();
        let back: SbomRecord = rmp_serde::from_slice(&bytes).unwrap();
        match back.source {
            SbomSource::Producer { origin } => assert_eq!(origin, "oci_referrer"),
            SbomSource::Generated { .. } => panic!("expected Producer"),
        }
    }

    #[test]
    fn sbom_record_roundtrip_generated() {
        let rec = SbomRecord {
            schema_version: CURRENT_RECORD_VERSION,
            target_blob_hash: "t".into(),
            sbom_blob_hash: "s".into(),
        ...
        let bytes = rmp_serde::to_vec(&rec).unwrap();
        let back: SbomRecord = rmp_serde::from_slice(&bytes).unwrap();
        assert!(matches!(back.source, SbomSource::Generated { .. }));
        assert_eq!(back.format, SbomFormat::Spdx);
    }

    #[test]
    fn oci_referrer_record_roundtrip() {
        let rec = OciReferrerRecord {
            schema_version: CURRENT_RECORD_VERSION,
            subject_digest: "sha256:aaa".into(),
            referrer_digest: "sha256:bbb".into(),
        ...
        let bytes = rmp_serde::to_vec(&rec).unwrap();
        let back: OciReferrerRecord = rmp_serde::from_slice(&bytes).unwrap();
        assert_eq!(back.subject_digest, "sha256:aaa");
        assert_eq!(back.size, 2048);
    }

    /// A v5 RepoConfig (no `scan_enabled` field) must deserialise under v6
    /// code, defaulting `scan_enabled` to `false`.
    #[test]
    fn repo_config_v5_backward_compat() {
        #[derive(Serialize)]
        struct RepoConfigV5 {
            schema_version: u32,
            name: String,
            ...
            store: String,
            created_at: DateTime<Utc>,
            cleanup_max_unaccessed_days: Option<u64>,
            cleanup_max_age_days: Option<u64>,
            deleting: bool,
        }
        let v5 = RepoConfigV5 {
            schema_version: 5,
        ...
        let bytes = rmp_serde::to_vec_named(&v5).unwrap();
        let v6: RepoConfig = rmp_serde::from_slice(&bytes).unwrap();
        assert_eq!(v6.name, "repo");
        assert!(
            !v6.scan_enabled,
            "v5 records must default scan_enabled=false"
        );
    }
}
